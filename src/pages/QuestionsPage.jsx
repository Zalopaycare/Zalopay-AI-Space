...
const ME = { name: 'NgocTA', initials: 'NT', team: 'Product Ops' }
...
const INITIAL_QUESTIONS = [
  {
    id: 'q1', title: 'Làm sao để dùng Claude tóm tắt file PDF dài hơn 50 trang?',
    body: 'Mình cần tóm tắt các hợp đồng dài, thử paste trực tiếp thì bị cắt nội dung ở giữa. Có cách nào chia nhỏ tài liệu mà vẫn giữ được mạch nội dung và các điều khoản quan trọng không?\n\nHiện tại mình làm thủ công: cắt theo chương rồi ghép lại, nhưng rất mất thời gian và dễ bỏ sót các điều khoản về giá.',
    author: 'NgocTA', initials: 'NT', team: 'Product Ops',
    category: 'Research & Knowledge', topics: ['Tài liệu dài', 'Tóm tắt'], tools: ['Claude'],
    time: '3 giờ trước', ts: 3, resolved: false, saved: false, files: [],
    images: [{ id: 'q1-img-1', placeholder: 'Ảnh chụp tài liệu hợp đồng mẫu' }],
    answers: [
      { id: 'a1', author: 'HaiPD', initials: 'HP', team: 'Data', time: '2 giờ trước', helpful: 8, iHelped: false, accepted: false, body: 'Chia theo cấu trúc chứ đừng chia theo số trang. Mình tách theo chương/điều khoản, tóm tắt từng phần với một prompt cố định, rồi cho model tổng hợp lại các bản tóm tắt ở bước cuối. Giữ nguyên tiêu đề chương trong mỗi phần để không mất mạch.', comments: [{ id: 'c1', author: 'QuyenNT', initials: 'QN', team: 'Marketing', time: '1 giờ trước', body: '@HaiPD bước tổng hợp cuối bạn dùng prompt gì để không bị lặp ý?' }] },
      { id: 'a2', author: 'DucMH', initials: 'DM', team: 'Engineering', time: '1 giờ trước', helpful: 3, iHelped: false, accepted: false, body: 'Bổ sung: bảo model trích dẫn nguyên văn các điều khoản về giá và thời hạn, kèm số trang. Đỡ phải mở lại file để kiểm tra.', comments: [] },
    ],
  },
  {
    id: 'q2', title: 'Prompt nào hiệu quả để sinh mô tả sản phẩm bằng tiếng Việt?',
    body: 'Mình thử vài prompt nhưng output còn cứng, chưa đúng tone thương hiệu. Mọi người có mẫu prompt nào đang dùng tốt cho tiếng Việt không?',
    author: 'QuyenNT', initials: 'QN', team: 'Marketing',
    category: 'Content & Communication', topics: ['Prompting', 'Tiếng Việt'], tools: ['ChatGPT', 'Gemini'],
    time: '1 ngày trước', ts: 24, resolved: true, saved: true, files: [],
    answers: [
      { id: 'a3', author: 'LinhVT', initials: 'LV', team: 'Brand', time: '20 giờ trước', helpful: 14, iHelped: false, accepted: true, body: 'Đưa cho model 3-5 đoạn mô tả cũ đã được duyệt làm ví dụ, kèm danh sách từ không được dùng. Tone đúng hơn nhiều so với việc mô tả tone bằng tính từ.', comments: [] },
      { id: 'a4', author: 'TrucVN', initials: 'TV', team: 'Customer Support', time: '18 giờ trước', helpful: 5, iHelped: false, accepted: false, body: 'Mình thêm một bước: yêu cầu model tự chấm điểm output theo checklist brand voice rồi viết lại bản tốt nhất.', comments: [] },
    ],
  },
  {
    id: 'q3', title: 'Có cách nào tự động phân loại ticket CSKH theo chủ đề?',
    body: 'Mỗi ngày team nhận khoảng 400 ticket, phân loại tay không kịp. Mình muốn gắn nhãn tự động rồi mới route sang các nhóm xử lý.',
    author: 'TrucVN', initials: 'TV', team: 'Customer Support',
    category: 'Automation & Workflow', topics: ['Ticket & CSKH'], tools: ['Gemini'],
    time: '2 ngày trước', ts: 48, resolved: false, saved: false, files: [],
    images: [
      { id: 'q3-img-1', placeholder: 'Ảnh chụp màn hình danh sách ticket' },
      { id: 'q3-img-2', placeholder: 'Ảnh bảng nhãn phân loại hiện tại' },
    ],
    answers: [],
  },
  {
    id: 'q4', title: 'Dùng AI review code có an toàn với repo nội bộ không?',
    body: 'Team mình muốn thử review tự động cho các PR nhỏ. Băn khoăn về việc gửi source lên dịch vụ ngoài, và nên giới hạn ở mức nào.',
    author: 'DucMH', initials: 'DM', team: 'Engineering',
    category: 'Coding & Technical', topics: ['Bảo mật dữ liệu', 'Code review'], tools: ['Copilot', 'Claude'],
    time: '4 ngày trước', ts: 96, resolved: false, saved: false, files: [], answers: [],
  },
]
...
export default function QuestionsPage() {
  ...
  const patch = (id, fn) => setQuestions((qs) => qs.map((q) => (q.id === id ? fn(q) : q)))

  const mentionList = (mq, apply) => PEOPLE.filter((p) => p.name !== ME.name && (!mq || p.name.toLowerCase().includes(mq)))
  ...
  useEffect(() => {
    ...
    if (!INITIAL_QUESTIONS.some((q) => q.id === id)) return
    setView('feed')
  ...
  const accept = (qid, aid) => patch(qid, (q) => ({ ...q, resolved: true, answers: q.answers.map((a) => ({ ...a, accepted: a.id === aid })) }))
  ...
  const postQuestion = () => {
    ...
    const id = 'q' + Date.now()
    setQuestions((qs) => [{
      id, title: tt, body: bb, author: ME.name, initials: ME.initials, team: ME.team,
      ...
      time: 'Vừa xong', ts: 0, resolved: false, saved: false, files: askFiles.slice(), answers: [],
    }, ...qs])
    setView('feed')
    setExpanded((s) => ({ ...s, [id]: true }))
  ...
  const match = (q) => {
    ...
    if (quick === 'mine' && q.author !== ME.name) return false
  ...
  const feed = list.map((q) => {
    ...
    const answers = ordered.map((a) => {
      ...
      return {
        ...
        canAccept: q.author === ME.name && !q.resolved,
        showComments: !!openComments[a.id] || a.comments.length > 0,
        comments: a.comments.map((c, j) => ({ ...c, bodyEl: renderBody(c.body), avatarBg: AV[(c.author.charCodeAt(0) + j) % AV.length] })),
        commentDraft: cd,
        mentionOpen: cm !== null,
        ...
        onPostComment: () => {
          ...
          if (!body) return
          patch(q.id, (x) => ({ ...x, answers: x.answers.map((y) => (y.id === a.id ? { ...y, comments: [...y.comments, { id: 'c' + Date.now(), author: ME.name, initials: ME.initials, team: ME.team, time: 'Vừa xong', body }] } : y)) }))
          setCommentDrafts((s) => ({ ...s, [a.id]: '' })); setCommentMention(null)
        },
        onReply: () => setOpenComments((s) => ({ ...s, [a.id]: !s[a.id] })),
        onHelpful: () => patch(q.id, (x) => ({ ...x, answers: x.answers.map((y) => (y.id === a.id ? { ...y, iHelped: !y.iHelped, helpful: y.helpful + (y.iHelped ? -1 : 1) } : y)) })),
    ...
    return {
      ...
      onHelpfulQ: (e) => { if (e) e.stopPropagation(); patch(q.id, (x) => ({ ...x, iHelpedQ: !x.iHelpedQ, qHelpful: (x.qHelpful || 0) + (x.iHelpedQ ? -1 : 1) })) },
      ...
      isOwner: q.author === ME.name && !q.resolved && q.answers.length > 0,
      onToggle: () => toggle(q.id),
      onExpandBody: () => setFullBody((s) => ({ ...s, [q.id]: !s[q.id] })),
      onSave: (e) => { if (e) e.stopPropagation(); patch(q.id, (x) => ({ ...x, saved: !x.saved })) },
      replyDraft: draft,
      ...
      onPostReply: () => {
        ...
        if (!body) return
        patch(q.id, (x) => ({ ...x, answers: [...x.answers, { id: 'a' + Date.now(), author: ME.name, initials: ME.initials, team: ME.team, time: 'Vừa xong', helpful: 0, iHelped: false, accepted: false, body, comments: [] }] }))
          ...
          <div style={css('padding:24px 40px 90px;')}>
            <div style={css('max-width:760px; margin:0 auto;')}>
              ...
              <div onClick={() => setView('ask')} className={hoverClass('background:rgba(255,255,255,.12); border-color:rgba(160,196,255,.5);')} style={css('display:flex; align-items:center; gap:14px; background:rgba(255,255,255,.07); backdrop-filter:blur(18px) saturate(140%); -webkit-backdrop-filter:blur(18px) saturate(140%); border:1px solid rgba(255,255,255,.16); border-radius:18px; padding:16px 18px; box-shadow:0 18px 40px rgba(0,0,0,.28); cursor:pointer; transition:background .25s,border-color .25s;')}>
                <div style={css('flex:none; width:42px; height:42px; border-radius:50%; background:linear-gradient(180deg,#4480ff,#2c5fff); color:#fff; display:flex; align-items:center; justify-content:center; font:800 14px "Aeonik Pro","Geist","Be Vietnam Pro",sans-serif;')}>NT</div>
                    ...
                    {q.expanded && (
                      <div style={css('margin-top:16px; padding:18px 22px 20px; background:#F8FAFE; border-top:1px solid #EEF1F7;')}>
                                ...
                                {a.showComments && (
                                  <div style={css('display:flex; flex-direction:column; gap:10px; margin-top:12px; padding-left:14px; border-left:2px solid #E2E8F5;')}>
                                    ...
                                    <div style={css('display:flex; gap:10px; align-items:center;')}>
                                      <div style={css('flex:none; width:26px; height:26px; border-radius:50%; background:linear-gradient(180deg,#4480ff,#2c5fff); color:#fff; display:flex; align-items:center; justify-content:center; font:800 10px "Aeonik Pro","Geist","Be Vietnam Pro",sans-serif;')}>NT</div>
                        ...
                        <div style={css('display:flex; gap:12px; margin-top:16px; padding-top:16px; border-top:1px solid #E6EBF3;')}>
                          <div style={css('flex:none; width:34px; height:34px; border-radius:50%; background:linear-gradient(180deg,#4480ff,#2c5fff); color:#fff; display:flex; align-items:center; justify-content:center; font:800 12px "Aeonik Pro","Geist","Be Vietnam Pro",sans-serif;')}>NT</div>
            ...
            <div onClick={(e) => e.stopPropagation()} style={css('width:760px; max-width:100%;')}>
              ...
              <div style={css('display:flex; flex-direction:column; gap:16px;')}>
                ...
                <div style={css('display:flex; align-items:center; gap:12px;')}>
                  <span style={css('font:600 12.5px "Aeonik Pro","Geist","Be Vietnam Pro",sans-serif; color:#94a3b8;')}>{t('Đăng dưới tên thật:')} <span style={css('color:#3A4757; font-weight:700;')}>Nguyễn Thảo · Product Ops</span></span>
