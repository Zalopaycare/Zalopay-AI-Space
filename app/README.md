...
The app talks to a real backend now — see `../server/README.md`. It's an
Express + SQLite service providing: email-code login (company domains only),
persisted questions/answers/comments/reactions, use-case submissions with an
admin approve/reject workflow, and email notifications (new answer, mention,
submission reviewed).

## Development

Run the backend first (in `../server`, see its README), then:
...
In production the backend serves the built frontend itself (single process,
single port) — see the root `Dockerfile`.
